export const soups = [
	{
		name: 'Chicken Noodle Soup',
		description:
			'Comforting slow-simmered chicken broth with tender egg noodles, shredded chicken, celery, and carrots.',
		price: 9.5,
		imageUrl: 'https://images.pexels.com/photos/4113896/pexels-photo-4113896.jpeg',
		images: [
			'https://images.pexels.com/photos/70497/pexels-photo-70497.jpeg',
			'https://images.pexels.com/photos/302680/pexels-photo-302680.jpeg',
			'https://images.pexels.com/photos/1640772/pexels-photo-1640772.jpeg',
			'https://images.pexels.com/photos/3026808/pexels-photo-3026808.jpeg',
			'https://images.pexels.com/photos/291528/pexels-photo-291528.jpeg',
		],
	},
	{
		name: 'Miso Soup',
		description:
			'Traditional Japanese dashi-based miso soup with silken tofu, wakame seaweed, and sliced green onions.',
		price: 6.0,
		imageUrl: 'https://images.pexels.com/photos/3026808/pexels-photo-3026808.jpeg',
		images: [
			'https://images.pexels.com/photos/70497/pexels-photo-70497.jpeg',
			'https://images.pexels.com/photos/1640772/pexels-photo-1640772.jpeg',
			'https://images.pexels.com/photos/291528/pexels-photo-291528.jpeg',
			'https://images.pexels.com/photos/414886/pexels-photo-414886.jpeg',
			'https://images.pexels.com/photos/302680/pexels-photo-302680.jpeg',
		],
	},
	{
		name: 'Tomato Soup',
		description:
			'Velvety roasted tomato soup blended with fresh basil and a touch of cream, served with crusty bread.',
		price: 8.5,
		imageUrl: 'https://images.pexels.com/photos/1640773/pexels-photo-1640773.jpeg',
		images: [
			'https://images.pexels.com/photos/70497/pexels-photo-70497.jpeg',
			'https://images.pexels.com/photos/3026808/pexels-photo-3026808.jpeg',
			'https://images.pexels.com/photos/616354/pexels-photo-616354.jpeg',
			'https://images.pexels.com/photos/1640772/pexels-photo-1640772.jpeg',
			'https://images.pexels.com/photos/4109994/pexels-photo-4109994.jpeg',
		],
	},
	{
		name: 'French Onion Soup',
		description:
			'Rich caramelized onion soup in a savory beef broth, topped with a toasted baguette slice and melted Gruyère.',
		price: 11.0,
		imageUrl: 'https://images.pexels.com/photos/2696064/pexels-photo-2696064.jpeg',
		images: [
			'https://images.pexels.com/photos/70497/pexels-photo-70497.jpeg',
			'https://images.pexels.com/photos/302680/pexels-photo-302680.jpeg',
			'https://images.pexels.com/photos/1640773/pexels-photo-1640773.jpeg',
			'https://images.pexels.com/photos/3026808/pexels-photo-3026808.jpeg',
			'https://images.pexels.com/photos/291528/pexels-photo-291528.jpeg',
		],
	},
	{
		name: 'Clam Chowder',
		description:
			'New England-style creamy clam chowder loaded with tender clams, potatoes, and smoky bacon in a thick cream broth.',
		price: 12.5,
		imageUrl: 'https://images.pexels.com/photos/5410415/pexels-photo-5410415.jpeg',
		images: [
			'https://images.pexels.com/photos/70497/pexels-photo-70497.jpeg',
			'https://images.pexels.com/photos/1640772/pexels-photo-1640772.jpeg',
			'https://images.pexels.com/photos/302680/pexels-photo-302680.jpeg',
			'https://images.pexels.com/photos/3026808/pexels-photo-3026808.jpeg',
			'https://images.pexels.com/photos/414886/pexels-photo-414886.jpeg',
		],
	},
	{
		name: 'Lentil Soup',
		description:
			'Hearty red lentil soup simmered with cumin, turmeric, and tomatoes, finished with a drizzle of lemon-herb oil.',
		price: 9.0,
		imageUrl: 'https://images.pexels.com/photos/5946704/pexels-photo-5946704.jpeg',
		images: [
			'https://images.pexels.com/photos/291528/pexels-photo-291528.jpeg',
			'https://images.pexels.com/photos/70497/pexels-photo-70497.jpeg',
			'https://images.pexels.com/photos/1640773/pexels-photo-1640773.jpeg',
			'https://images.pexels.com/photos/302680/pexels-photo-302680.jpeg',
			'https://images.pexels.com/photos/1640772/pexels-photo-1640772.jpeg',
		],
	},
	{
		name: 'Cream of Mushroom',
		description:
			'Luxuriously smooth wild mushroom bisque made with a blend of cremini and shiitake mushrooms in a rich cream base.',
		price: 10.5,
		imageUrl: 'https://images.pexels.com/photos/1427861/pexels-photo-1427861.jpeg',
		images: [
			'https://images.pexels.com/photos/70497/pexels-photo-70497.jpeg',
			'https://images.pexels.com/photos/3026808/pexels-photo-3026808.jpeg',
			'https://images.pexels.com/photos/291528/pexels-photo-291528.jpeg',
			'https://images.pexels.com/photos/1640773/pexels-photo-1640773.jpeg',
			'https://images.pexels.com/photos/414886/pexels-photo-414886.jpeg',
		],
	},
	{
		name: 'Pho',
		description:
			'Aromatic Vietnamese beef bone broth poured over rice noodles and thinly sliced beef, served with fresh herbs and bean sprouts.',
		price: 14.0,
		imageUrl: 'https://images.pexels.com/photos/2773600/pexels-photo-2773600.jpeg',
		images: [
			'https://images.pexels.com/photos/70497/pexels-photo-70497.jpeg',
			'https://images.pexels.com/photos/302680/pexels-photo-302680.jpeg',
			'https://images.pexels.com/photos/3026808/pexels-photo-3026808.jpeg',
			'https://images.pexels.com/photos/616354/pexels-photo-616354.jpeg',
			'https://images.pexels.com/photos/1640773/pexels-photo-1640773.jpeg',
		],
	},
	{
		name: 'Minestrone',
		description:
			'Classic Italian vegetable minestrone with cannellini beans, seasonal vegetables, pasta, and a generous dusting of Parmesan.',
		price: 10.0,
		imageUrl: 'https://images.pexels.com/photos/8885744/pexels-photo-8885744.jpeg',
		images: [
			'https://images.pexels.com/photos/70497/pexels-photo-70497.jpeg',
			'https://images.pexels.com/photos/291528/pexels-photo-291528.jpeg',
			'https://images.pexels.com/photos/1640773/pexels-photo-1640773.jpeg',
			'https://images.pexels.com/photos/302680/pexels-photo-302680.jpeg',
			'https://images.pexels.com/photos/3026808/pexels-photo-3026808.jpeg',
		],
	},
	{
		name: 'Butternut Squash Soup',
		description:
			'Silky roasted butternut squash soup seasoned with warm autumn spices and topped with toasted pepitas and crème fraîche.',
		price: 10.0,
		imageUrl: 'https://images.pexels.com/photos/1640772/pexels-photo-1640772.jpeg',
		images: [
			'https://images.pexels.com/photos/70497/pexels-photo-70497.jpeg',
			'https://images.pexels.com/photos/291528/pexels-photo-291528.jpeg',
			'https://images.pexels.com/photos/3026808/pexels-photo-3026808.jpeg',
			'https://images.pexels.com/photos/414886/pexels-photo-414886.jpeg',
			'https://images.pexels.com/photos/1640773/pexels-photo-1640773.jpeg',
		],
	},
];
