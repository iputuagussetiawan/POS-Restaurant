export const desserts = [
	{
		name: 'Chocolate Cake',
		description: 'Rich triple-layer chocolate cake with ganache frosting and fresh berries.',
		price: 8.99,
		imageUrl: 'https://images.pexels.com/photos/291528/pexels-photo-291528.jpeg',
		images: [
			'https://images.pexels.com/photos/1854652/pexels-photo-1854652.jpeg',
			'https://images.pexels.com/photos/3026804/pexels-photo-3026804.jpeg',
		],
	},
	{
		name: 'Ice Cream Sundae',
		description:
			'Three scoops of vanilla ice cream with hot fudge, whipped cream, nuts and a cherry.',
		price: 7.49,
		imageUrl: 'https://images.pexels.com/photos/4109994/pexels-photo-4109994.jpeg',
		images: ['https://images.pexels.com/photos/1352278/pexels-photo-1352278.jpeg'],
	},
	{
		name: 'Tiramisu',
		description:
			'Classic Italian dessert with espresso-soaked ladyfingers, mascarpone and cocoa.',
		price: 9.49,
		imageUrl: 'https://images.pexels.com/photos/3026808/pexels-photo-3026808.jpeg',
		images: [],
	},
	{
		name: 'Cheesecake',
		description:
			'New York-style baked cheesecake on a graham cracker crust with berry compote.',
		price: 8.49,
		imageUrl: 'https://images.pexels.com/photos/7045693/pexels-photo-7045693.jpeg',
		images: ['https://images.pexels.com/photos/1126359/pexels-photo-1126359.jpeg'],
	},
	{
		name: 'Brownies',
		description: 'Fudgy dark-chocolate brownies served warm with a scoop of vanilla ice cream.',
		price: 6.99,
		imageUrl: 'https://images.pexels.com/photos/108370/pexels-photo-108370.jpeg',
		images: [],
	},
	{
		name: 'Fruit Tart',
		description:
			'Buttery pastry shell filled with vanilla custard and topped with seasonal fresh fruit.',
		price: 8.99,
		imageUrl: 'https://images.pexels.com/photos/3026809/pexels-photo-3026809.jpeg',
		images: [],
	},
	{
		name: 'Crème Brûlée',
		description: 'Silky vanilla custard with a perfectly caramelised sugar crust.',
		price: 9.99,
		imageUrl: 'https://images.pexels.com/photos/533325/pexels-photo-533325.jpeg',
		images: ['https://images.pexels.com/photos/5419336/pexels-photo-5419336.jpeg'],
	},
	{
		name: 'Donuts',
		description:
			'Freshly fried ring donuts glazed with your choice of chocolate, strawberry or original glaze.',
		price: 5.99,
		imageUrl: 'https://images.pexels.com/photos/3026806/pexels-photo-3026806.jpeg',
		images: [],
	},
	{
		name: 'Macarons',
		description:
			'Box of six assorted French macarons — raspberry, pistachio, salted caramel, chocolate, lemon and vanilla.',
		price: 10.49,
		imageUrl: 'https://images.pexels.com/photos/1571784/pexels-photo-1571784.jpeg',
		images: ['https://images.pexels.com/photos/239578/pexels-photo-239578.jpeg'],
	},
	{
		name: 'Panna Cotta',
		description: 'Silky Italian panna cotta with a coulis of mixed berries.',
		price: 7.99,
		imageUrl: 'https://images.pexels.com/photos/7045694/pexels-photo-7045694.jpeg',
		images: [],
	},
];
